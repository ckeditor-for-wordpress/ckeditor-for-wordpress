			...
			CKEDITOR.on('dialogDefinition', function(ev) {
				var dialogName = ev.data.name,
                                ...
                                var loaderLinkbrowser = function(url) {
                                        searchPanel.load(url, {action: 'linkbrowser_loader'}, function() {
                                                searchField = $("#search-field", searchPanel).css({'float': 'left', 'margin-top': '5px'});
                                ...
                                var loaderData = function(url, query, append) {
                                    ...
                                    var self = this;
                                    $.ajax({type: 'POST', dataType: 'json', url: url, data: query}).done(function(data) {
                                            if (data == false) {
                                                    searchWaiting.hide();
                                                    searchScrollWaiting.hide();
                                                    $(searchResults).unbind('scroll');
                                ...
                                //add new tab
				if (dialogName == "link" && dialogDefinition.getContents('linkBrowserTab') == null) {
